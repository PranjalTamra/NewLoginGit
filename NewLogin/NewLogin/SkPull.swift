...
open class LKPullToLoadMore {
    ...
    open func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if !loadingMore && enabled {
            ...
            if tableView.contentInset.bottom != 0 {
                tableView.contentInset = UIEdgeInsets(top: tableView.contentInset.top, left: tableView.contentInset.left, bottom: 0, right: tableView.contentInset.right)
            }
        }
    }
    ...
    func animateLoadingIndicator() {
        if loadingMore {
            UIView.animate(
                ...
                animations: {
                    self.loadMoreIndicator.transform = self.loadMoreIndicator.transform.rotated(by: .pi/2)
                },
    ...
    */
    func drawReloadIndicator(wedgeAngle: CGFloat) -> UIImage {
        ...
        ovalPath.addArc(withCenter: CGPoint(x: ovalRect.midX, y: ovalRect.midY), radius: ovalRect.width / 2, startAngle: 0 * .pi/180, endAngle: -wedgeAngle * .pi/180, clockwise: true)
